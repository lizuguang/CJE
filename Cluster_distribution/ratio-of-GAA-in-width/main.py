...
def figure(rate_wifi_my, rate_wifi_oth, wifi_density, wifiset_num):
    ...
    # -------------两种算法能够支持的用户比例----------------------------------
    ax.set_ylabel(r'Ratio of GAA users licensed $k$', font1)
    x = []
    for i in range(1,6):
        x.append(i * 10)
    ...
    lns1 = ax.plot(x, rate_wifi_my, ls='-', color='orangered', marker="o", markersize=10, label= r"$k$ in proposed algorithm")
    lns2 = ax.plot(x, rate_wifi_oth, ls='-', color='blue', marker="^", markersize=10, label= r"$k$ in WIF scheme")
    ax.set_ylim(0, 1.0)
    ...
    ax1.set_xlabel('Area Width $d (km)$')
    ax1.plot(x, wifi_density, ls='-', color='darkgoldenrod', marker="o", markersize=6, label="Density of GAA users")
    x_ticks = np.arange(10, 60, 10)
    plt.xticks(x_ticks)
    ax1.legend(loc = 0)
    ax1.grid()'''
    # ***********************
    ax2 = ax.twinx()
    lns3 = ax2.plot(x, wifi_density, ls='--', color='forestgreen', marker="d", markersize=10, label= r"$\rho$")
    lns = lns1 + lns2 +lns3
    labs = [l.get_label() for l in lns]
    ax.legend(lns, labs, loc = 0, prop=font2, framealpha=0.8)
    ax2.set_ylabel(r"Density of GAA users $\rho \ (/km^2)$", font1)
    ax2.set_ylim(0, 10)
    ax.grid()
    ax.margins(0)
    plt.savefig('fix.eps', dpi=300)
    plt.show()

def rate_wifi(d, filename, N):
    ...
    for stepwidth in range(1, 6):  # 初始话的步长为1，每增加1步长，正方形边长增加10km，正方形的初始化边长为10km stepwidth = 1~5
        ...
        # ************************************************************************************************************
        rate_wifi_my.append((sum(wifi_lice_num) + wifi_one_num) / wifi_num)
        rate_wifi_oth.append(wifi_lice_num2 / wifi_num)
...
if __name__ == '__main__':
    ...
    rate_wifi_my, rate_wifi_oth, wifi_density, wifiset_num = rate_wifi(d, filename, N)
    print('my: %s', sum(rate_wifi_my) / 5)
    print('oth: %s', sum(rate_wifi_oth) / 5)
    fig, ax = plt.subplots()
